                ...
                <tr>
                    ...
                    <td>
                        ...
                        @if($rental->status === 'pending')
                            <div class="d-flex gap-1 mt-2 flex-wrap">
                                <form method="POST" action="{{ route('owner.rentals.accept', $rental) }}">
                                    @csrf
                                    <button type="submit" class="btn btn-sm btn-success">Accept</button>
                                </form>
                                <form method="POST" action="{{ route('owner.rentals.reject', $rental) }}">
                                    @csrf
                    ...
                    <td>
                        @if($rental->lease_status === 'requested')
                            <div class="d-flex flex-column gap-2">
                                <form method="POST" action="{{ route('owner.rentals.lease.upload', $rental) }}" enctype="multipart/form-data">
                                    @csrf
                                    <input type="file" name="lease_file" class="form-control form-control-sm mb-1" accept=".pdf,.jpg,.jpeg,.png" required>
                                    <button type="submit" class="btn btn-sm btn-outline-primary w-100">Upload Lease</button>
                                </form>
                                @if($rental->leaseAgreement)
                                    <a class="btn btn-sm btn-light w-100"
                                       href="{{ route('rentals.lease.download', $rental->leaseAgreement) }}">View Lease</a>
                                @endif
                                <div class="d-flex gap-1 flex-wrap">
                                    ...
                                    <form method="POST" action="{{ route('owner.rentals.lease.reject', $rental) }}">
                                        @csrf
                                        <button type="submit" class="btn btn-sm btn-outline-danger">Reject</button>
                                    </form>
                                </div>
                        ...
                        @elseif($rental->lease_status === 'approved')
                            ...
                            @if($rental->leaseAgreement)
                                <div class="mt-1">
                                    <a class="btn btn-sm btn-light" href="{{ route('rentals.lease.download', $rental->leaseAgreement) }}">Lease File</a>
                                </div>
                            @endif
